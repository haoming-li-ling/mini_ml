...
structure Closure : sig
   ...
   *)
    ...
    and expToJump (prefix, env, S.E(_, S.E_RET v), JOIN k) =
           ...
           of S.E_LET(x, S.R_EXP(rhsExp as S.E(_, rhs)), scope) => (case rhs
                 of S.E_LET _ => raise Fail "impossible: nested let"
                  ...
                  | S.E_COND(tst, vs, eThen, eElse) => let
                      ...
                      val joinLive' = List.filter (fn y => not (SV.same (x, y))) joinLive
                      val joinLab = CFGLabel.new "join"
                      val joinParams = x :: joinLive'
                      val joinFrag = expToFrag (env, joinLab, joinParams,
                        scope, cont)
                      val join = JOIN (fn (env, v) => (joinLab, v ::
                          lookupVars (env, joinLive')))
                      val j1 = expToJump ("then", env, eThen, join)
                      val j2 = expToJump ("else", env, eElse, join)
                      in
                ...
                (* end case *))
            | S.E_LET(x, S.R_PRIM(p, vs), scope) => let
                val (x', env') = bindNewVar (env, x)
                val vs' = cvtVals (env', vs)
                in
                  CFG.mkLET (x', CFG.PRIM (p, vs'), cvtExp (env', scope,
                        cont))
                end
            | S.E_LET(x, S.R_CALL(cf, vs), scope) => let
                val (x', env') = bindNewVar (env, x)
                val vs' = cvtVals (env', vs)
                in 
                  CFG.mkLET (x', CFG.CALL (cf, vs'), cvtExp (env', 
                        scope, cont))
                end
            | S.E_LET(x, S.R_TUPLE[], scope) => let 
                val env' = bindVar (env, x, CFG.INT (IntInf.fromInt 0))
                in
                  cvtExp (env', scope, cont)
                end
            | S.E_LET(x, S.R_TUPLE vs, scope) => let
                val (x', env') = bindNewVar (env, x)
                val vs' = cvtVals (env, vs)
                in 
                  CFG.mkLET (x', CFG.ALLOC vs', cvtExp (env', 
                        scope, cont))
                end
            | S.E_LET(x, S.R_SELECT(i, y), scope) => let
                val (x', env') = bindNewVar (env, x)
                val v' = CFG.VAR (cvtVar (env, y))
                in 
                  CFG.mkLET (x', CFG.SEL (i, v'), cvtExp (env', scope,
                        cont))
                end
            | S.E_LET(x, S.R_DCON(dc, v), scope) => (
                case SDC.repOf dc
                  of SDC.Box => let 
                      val (x', env') = bindNewVar (env, x)
                      val v' = cvtVal env v
                      in CFG.mkLET (x', CFG.ALLOC [v'], cvtExp (env',
                          scope, cont))
                      end
                   | SDC.TaggedBox i => let
                      val (x', env') = bindNewVar (env, x)
                      val v' = cvtVal env v
                      in CFG.mkLET (x', CFG.ALLOC [CFG.INT
                          (IntInf.fromInt i), v'], cvtExp (env',
                            scope, cont))
                      end
                    | SDC.Enum i => raise Fail "impossible"
                    | SDC.Immediate => let 
                      val env' = bindVar (env, x, cvtVal env v)
                      val v' = cvtVal env v
                      in 
                        cvtExp (env', scope, cont)
                      end)
            | S.E_FUN(f, xs, body, e) => let
                val (f', env') = bindNewVar (env, f)
                val flab' = CFGLabel.new (SimpleVar.toString f) 
                val xs' = List.map (fn x => CFG.VAR (newVar x)) xs
                val fvs = VA.freeVarsOf f
                val fvs' = lookupVars (env, fvs) 
                val _ = cvtFunc (env, f, xs, body, fvs)
                val flab = getLabel f
                in
                  CFG.mkLET (f', CFG.ALLOC (CFG.CODE flab :: fvs'), 
                      cvtExp (env', e, cont))
                end
            | S.E_APPLY(f, vs) => if SV.same (f, Env.currentFun env)
                then let
                  val hdrLabel = Env.getHeaderLable env
                  val fvs = VA.freeVarsOf f
                  val fvs' = lookupVars (env, fvs)
                  in
                    CFG.mkGOTO (hdrLabel, cvtVal env (S.V_VAR f) ::
                        (cvtVals (env, vs) @ fvs'))
                  end
                else cvtApply (env, f, vs, CFG.mkTAIL_APPLY)
            | S.E_COND(tst, vs, eThen, eElse) => let
   ...
   *)
    and cvtCaseExp (env, v, argKind, rules, cont) = 
          if argKind = PrimType.Unboxed then 
            case rules
              of [] => raise Fail "impossible"
               | [(S.P_CONST d, e)] => cvtExp (env, e, cont)
               | (S.P_CONST d, e) :: rs => let
                  val SDC.Enum n = SDC.repOf d
                  val j1 = expToJump ("then", env, e, cont)
                  fun liveRules [] = []
                    | liveRules ((_, e) :: rs) = 
                        VA.liveVarsAt e @ liveRules rs
                  val liveVarsUnion = liveRules rs
                  val (liveUnion, env') = mkParams (env, liveVarsUnion)
                  val flab = CFGLabel.new "else"
                  val fragRS = CFGFrag.new (flab, liveUnion, 
                        cvtCaseExp (env', v, argKind, rs, cont))
                  val _ = Env.addFrag (env, fragRS)
                  val j2 = (flab, lookupVars (env, liveVarsUnion)) 
                  in
                    CFG.mkIF (PrimCond.IntEq, 
                        [v, CFG.INT (IntInf.fromInt n)], j1, j2)
                  end
            ...
            case rules 
              of [] => raise Fail "impossible"
               | [(S.P_DCON (d, p), e)] => (
                   case SDC.repOf d 
                     of SDC.Immediate => let
                        val env' = bindVar (env, p, v)
                        in 
                          cvtExp (env', e, cont)
                        end
                      | SDC.Box => let
                        val (p', env') = bindNewVar (env, p)
                        in 
                          CFG.mkLET (p', CFG.SEL (0, v), 
                              cvtExp (env', e, cont))
                        end
                      | SDC.TaggedBox n => let
                        val (p', env') = bindNewVar (env, p)
                        in 
                          CFG.mkLET (p', CFG.SEL (1, v), 
                              cvtExp (env', e, cont))
                        end
                      | _ => raise Fail "impossible"
                      )
               | (S.P_DCON (d, p), e) :: rs => let
                  val SDC.TaggedBox n = SDC.repOf d
                  val (p', env1) = bindNewVar (env, p)
                  val liveVars = VA.liveVarsAt e
                  val (params1, env') = mkParams (env, liveVars)
                  val flab1 = CFGLabel.new "then"
                  val fragThen = CFGFrag.new (flab1, params1, 
                        CFG.mkLET (p', CFG.SEL (1, v), 
                        cvtExp (env1, e, cont)))
                  val _ = Env.addFrag (env, fragThen)
                  val j1 = (flab1, lookupVars (env, liveVars))
                  fun liveRules [] = []
                    | liveRules ((_, e) :: rs) = 
                        VA.liveVarsAt e @ liveRules rs
                  val liveVarsUnion = liveRules rs
                  val (liveUnion, env2) = mkParams (env, liveVarsUnion)
                  val flab2 = CFGLabel.new ("after rule " ^ SDC.nameOf d)
                  val fragElse = CFGFrag.new (flab2, liveUnion,
                        cvtCaseExp (env2, v, argKind, rs, cont))
                  val _ = Env.addFrag (env, fragElse)
                  val j2 = (flab2, lookupVars (env, liveVarsUnion)) 
                  in
                    CFG.mkIF (PrimCond.IntEq, 
                        [v, CFG.INT (IntInf.fromInt n)], j1, j2)
                  end
