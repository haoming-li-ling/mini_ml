...
(* FIXME: should not be true for entry fuction, but we fix it in the LLVM library *)
        ...
        (* compute and record liveness information for a fragment *)
          fun liveness frag = let
                val e = CFGFrag.bodyOf frag
                fun liveExp (CFG.EXP (pt, CFG.LET (v, rhs, e'))) = let
                      val () = liveExp e'
                      val CFG.EXP (pt', _) = e'
                      val livee' = getLive pt'
                      val liverhs = liveRhs rhs
                      val u = Set.subtract (Set.addList (liverhs, livee'), v)
                      in 
                        setLive (pt, u)
                      end
                  | liveExp 
                      (CFG.EXP (pt, CFG.IF (_, vl, (_, vl1), (_, vl2)))) = 
                      let
                        val s1 = Set.fromList (valsToVars vl)
                        val s2 = Set.fromList (valsToVars vl1)
                        val s3 = Set.fromList (valsToVars vl2)
                        val u = Set.union (Set.union (s1, s2), s3)
                      in
                        setLive (pt, u)
                      end
                  | liveExp (CFG.EXP (pt, CFG.TAIL_APPLY (_, v, vl))) =  
                      setLive (pt, Set.fromList (valsToVars vl))
                  | liveExp (CFG.EXP (pt, CFG.GOTO (_, vl))) = 
                      setLive (pt, Set.fromList (valsToVars vl))
                  | liveExp (CFG.EXP (pt, CFG.RETURN v)) = setLive (pt,
                    Set.fromList (valsToVars [v]))
                and liveRhs (CFG.CALL (_, vl)) = Set.fromList (valsToVars vl)
                  | liveRhs (CFG.APPLY (ty, v, vl)) = Set.fromList
                  (valsToVars (v :: vl))
                  | liveRhs (CFG.PRIM (_, vl)) = Set.fromList (valsToVars vl)
                  | liveRhs (CFG.ALLOC vl) = Set.fromList (valsToVars vl)
                  | liveRhs (CFG.SEL (_, v)) = Set.fromList (valsToVars [v])
                and valsToVars [] = []
                  | valsToVars (v :: vs) = (
                      case v 
                        of CFG.VAR v' => v' :: valsToVars vs 
                         | _ => valsToVars vs)
                in
                  liveExp e
                end
