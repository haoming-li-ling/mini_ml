...
structure Context : sig
  ...
  (* add a data constructor binding to the context *)
    val bindCon : t * Atom.atom * BindTree.conid -> t
  ...
  (* return the varEnv field of a context *)
    val getVarEnv : t -> var_env
    
  (* return the conEnv field of a context *)
    val getConEnv : t -> dcon_env

  (* empty the varEnv field of a context *)
    val clearVarEnv : t -> t

  (* empty the conEnv field of a context *)
    val clearConEnv : t -> t
  ...
  (* print the environments of a context to stdOut (for debugging) *)
    ...
    fun getConEnv (Cxt {conEnv, ...}) = conEnv

    fun getVarEnv (Cxt {varEnv, ...}) = varEnv

    fun clearVarEnv (Cxt stuff) = Cxt {
            errStrm = #errStrm stuff,
            span = #span stuff,
            tvEnv = #tvEnv stuff,
            tycEnv = #tycEnv stuff,
    ...
    fun bindCon (Cxt stuff, x, x') = Cxt{
            errStrm = #errStrm stuff,
            span = #span stuff,
            tvEnv = #tvEnv stuff,
            tycEnv = #tycEnv stuff,
            conEnv = AMap.insert (#conEnv stuff, x, x'),
            varEnv = #varEnv stuff
          }
